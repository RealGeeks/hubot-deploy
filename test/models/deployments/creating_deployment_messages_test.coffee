...
describe "Deployment#post", () ->
  ...
  it "does not create a deployment due to missing required commit statuses", (done) ->
    ...
    deployment.post (err, status, body, headers, message) ->
      ...
      assert.equal "Unmet required commit status contexts for hubot-deploy: continuous-integration/travis-ci/push failed.\nI'll ping you when it's ready to deploy!", message
      done()
  ...
  it "does not create a deployment due to failing required commit statuses", (done) ->
    ...
    deployment.post (err, status, body, headers, message) ->
      ...
      assert.equal "Unmet required commit status contexts for hubot-deploy: continuous-integration/travis-ci/push,code-climate failed.\nI'll ping you when it's ready to deploy!", message
      done()
